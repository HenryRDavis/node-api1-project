...
let users = [
  ...
  { id: 2, name: "Nou", Bio: "Big" },
  { id: 3, name: "No", Bio: "Sad" },
];
let nextId = 4;
...
server.get("/api/users", (req, res) => {
  try {
    ...
    res.status(500).json({
      errorMessage: "Could not retrieve info",
      error,
    });
  }
});
...
server.delete("/api/users/:id", (req, res) => {
  const id = Number(req.params.id);
  const iD = users.some((item) => item.id === id);
    if (iD) {
    ...
    res.status(500).json({ errorMessage: "The user could could not be removed" });
});
